...
#import cv2
#resnet-110-rnn-sp-cifar10.pth.tar
...
def parse_args():
    ...
    parser.add_argument('--gate-type', type=str, default='rnn',
    ...
    parser.add_argument('--resume', default='resnet-110-rnn-cifar10.pth.tar', type=str,
...
def add_median_fliter(image):
    new_image = cv2.blur(image, (2, 2))
...
def normalize(p_list):
    ...
    for i in range(3):
        for j in range(32):
            for k in range(32):
                img[i][j][k]=(((p_list[i][j][k]/255)- mean[i])/std[i])
...
def validate(args, test_loader, model, criterion):
    ...
    flop_list=flop_list1+flop_list2+flop_list2
    ...
    for i, (input, target) in enumerate(test_loader):
        try:
            ...
            if i > threshold and i< (threshold+10000):
                ...
                for j in range(1):
                    ...
                    current_target = target.tolist()[j]
                    print(input_var.is_cuda)

                    x, masks, gprobs = model(input_var)
                    ...
                    lst2 = np.zeros((54, 32, 32), dtype=np.float64)
                    ind=0
                    #print(gprobs)
                    '''for g in gprobs:
                    ...
                    for g in gprobs:
                        ...
                        ind+=1
                    print(pr)
                    np.save('sal_map2/orig_saliency_skip_gts_rl_' + '_' + str(i) + '.npy', lst2)

                    f = open("block_details_skip2.csv", "a")
                    writer = csv.writer(f)
                    # print(energy)
                    ...
                    if (sum > 0.0):
                        ...
                        sm=t1+t2+t3
                        '''open_cv_image=create_image(py_list, i, j, sum, t1, t2, t3, "orig", 0, "0")
                        open_cv_image1,open_cv_image2,open_cv_image3=add_median_fliter(open_cv_image)
                        open_cv_image=normalize(open_cv_image1)
                        py_list[j]=open_cv_image.tolist()
                        ...
                        open_cv_image = normalize(open_cv_image2)
                        py_list[j] = open_cv_image.tolist()
                        input_var = torch.as_tensor(py_list)
                        probs, probs2, stacked_tensor, stacked_tensor2 = model(input_var)
                        sum = 0
                        ...
                        open_cv_image = normalize(open_cv_image3)
                        py_list[j] = open_cv_image.tolist()
                        input_var = torch.as_tensor(py_list)
                        probs, probs2, stacked_tensor, stacked_tensor2 = model(input_var)
                        sum = 0
                        ...
                        file_object.write(str(gates[0]) + " " + str(gates[1]) + " " + str(gates[2]) + " " + str(
                            gates[3]) + "\n")
                        file_object.close()
                        tot_gates.append(gates)'''
                        border_init = datetime.utcnow()
                        ...
                        # f2 = np.vectorize(fun)
                        # constraints=cons
                        res1 = minimize(fun, dis2, args=(input_var, j), method='SLSQP', bounds=bnds, constraints=cons,
                                        options={'maxiter': 10})
                        # res = fmin(func2, dis, args=(input_var, model))
                        border_end = datetime.utcnow()
                        print("res1 ", res1.x)
                        isNaN = False
                        for tmp in res1.x:
                        ...
                        if isNaN is False:
                            ...
                            create_image(py_list, i, j, res, cnt1, cnt2, cnt3, "borderline", inc_val,
                                         str(border_end - border_init))
                        static_init = datetime.utcnow()
                        ...
                        if math.isnan(res1.x[0]) is False:
                            ...
                            create_image(py_list, i, j, res, cnt1, cnt2, cnt3, "static", inc_val,
                                         str(static_end - static_init))'''

                        cw_init = datetime.utcnow()
                        modifier = torch.zeros(input_var.size(),device='cuda').float()
                        ...
                        for index in range(53):
                            target.append(torch.as_tensor([0.5],device='cuda'))
                        '''for index in range(54):
                            target.append(torch.as_tensor([0.5], device='cuda'))'''
                            #target.append(torch.as_tensor([0.5]))
                        #target_var = autograd.Variable(target, requires_grad=False)
                        min_loss = sys.float_info.max
                        adv_img_min = numpy.zeros((1, 3, 32, 32))
                        ...
                        #sum2, t1, t2, t3 = create_sum(min_output)
                        ...
                        t2=0
                        t3=0
                        for ind in range(len(min_output)):
                            if (min_output[ind] > 0.5):
                        ...
                        sm2 = t1 + t2 + t3
                        adv_img = adv_img_min
                        #numpy.save('np/'+str(i)+'_'+str(j)+'_'+str(current_target)+'_'+str(sm)+'_'+str(sm2)+'_data.npy', adv_img)

                        adv_clone=torch.from_numpy(adv_img)
                        adv_clone=Variable(adv_clone, requires_grad=True)
                        adv_clone.retain_grad()

                        x, masks, gprobs = model(adv_clone)
                        pr = []
                        ...
                        for g in gprobs:
                            ...
                            ind += 1
                        print(pr)
                        np.save('sal_map2/best_saliency_skip_gts_rl' + '_' + str(i) + '.npy', lst2)
                        ...
                        new_im.save(
                            "output/img_" + str(i) + "_" + str(j) + "_" + str(sum) + "_" + str(t1) + "_" + str(
                                t2) + "_" + str(
                                t3) + "_" + "cw" + "_" + str(inc_val) + "_" + str(cw_end - cw_init) + ".png")'''
...
def fun(dis2,input_var,num):
    ...
    for i in range(3):
        for j in range(32):
            rem=j%16
            for k in range(1):
                sum+=abs(py_list[num][i][j][k]-dis2[rem])
...
def create_image(py_list,i,j,sum,t1,t2,t3,type,val,time):
    ...
    for i1 in range(3):
        for j1 in range(32):
            for k1 in range(32):
                img[i1][j1][k1]=int((p_list[i1][j1][k1]*std[i1]+mean[i1])* 255)
    ...
    # Convert RGB to BGR
    open_cv_image = img_arr[:, :, ::-1].copy()
    return open_cv_image
    #new_im.save("output/img_" + str(i) + "_" + str(j) + "_" + str(sum) +"_"+str(t1)+"_"+str(t2)+"_"+str(t3)+"_"+type+"_"+str(val)+"_"+time+ ".png")
...
def dist2(py_list,num,dis2):
    for i in range(3):
        for j in range(32):
            for k in range(32):
                py_list[num][i][j][k] = py_list[num][i][j][k] + dis2
...
def loss_op(output, target, dist, scale_const):
    ...
    loss1=torch.as_tensor(0.0,device='cuda')
    #loss1 = torch.as_tensor(0.0)
    ...
    for index in range(53):
        #print(torch.clamp(target[index] - output[index], min=0.))

        loss1=torch.add(torch.clamp(target[index] - output[index][1], min=0.),loss1)
        #loss1=torch.add(torch.clamp(target[index] - output[index], min=0.),loss1)
...
def optimize(optimizer, model, input_var, modifier_var, target_var, j):
    ...
    model = remove_dropout(model)
    x, masks, gprobs = model(input_adv)


    dist = l2_dist(input_adv, input_var, keepdim=False)

    loss = loss_op(gprobs, target_var, dist, scale_const_var)
    output_np = []

    for g in gprobs:
        output_np.append(g.tolist()[0])
